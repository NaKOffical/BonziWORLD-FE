(function (window, undefined) {
	...
	speak.play = function (text, args, onended, onstart) {
		...
		function handleWav(wav) {
			...
			playSound(buffer);
			if (PROFILE) console.log('wav ' + (Date.now() - startTime).toFixed(2) + ' ms');
		}
		...
		if (args && args.noWorker) {
			...
			if (PROFILE) console.log('Process ' + (Date.now() - startTime).toFixed(2) + ' ms');
			playSound(wav);
		} else {
			// Call the worker, which will return a wav that we then play
			var startTime = Date.now();
			speakWorker.onmessage = function (event) {
				if (PROFILE) console.log('Worker ' + (Date.now() - startTime).toFixed(2) + ' ms');
